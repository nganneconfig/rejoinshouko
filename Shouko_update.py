...
class PackageSelector:
    @staticmethod
    def display_package_list(accounts, server_links):
        """Display package selection interface similar to rejoin.cjs.txt"""
        print("\n📋 Danh sách packages đã cấu hình:")
        
        # Create a mapping of package_name to server_link for quick lookup
        server_link_map = dict(server_links)
        
        # Display packages in a numbered list
        package_list = []
        index = 1
        for package_name, user_id in accounts:
            server_link = server_link_map.get(package_name, "Not configured")
            username = FileManager.get_username(user_id)
            
            # Mask sensitive information
            masked_username = "******" + username[6:] if len(username) > 6 else "******"
            
            # Determine package display name based on package name
            prefix = globals().get("package_prefix", "com.roblox")
            if package_name == f"{prefix}.client":
                package_display = 'Global 🌍'
            elif package_name == f"{prefix}.client.vnggames":
                package_display = 'VNG 🇻🇳'
            else:
                package_display = package_name
            
            print(f"{index}. {package_display} ({masked_username}) - Game: {server_link}")
            package_list.append((package_name, user_id, server_link))
            index += 1
        
        return package_list

    @staticmethod
    def select_packages(package_list):
        """Allow user to select specific packages or all packages"""
        print("\n🎯 Chọn packages để rejoin:")
        print("0. 🚀 Chạy tất cả packages")
        print("Hoặc nhập số thứ tự packages cách nhau bằng khoảng trắng (ví dụ: 1 3 5)")
        ...
        while True:
            try:
                ...
                if choice == "0":
                    ...
                    # Parse individual package selections
                    indices = [int(x) - 1 for x in choice.split() if x.isdigit()]
                    selected_packages = []
                    
                    for idx in indices:
                        if 0 <= idx < len(package_list):
                            selected_packages.append(package_list[idx])
                    
                    if selected_packages:
                        print(f"🎯 Sẽ chạy {len(selected_packages)} packages:")
                        for pkg in selected_packages:
                            prefix = globals().get("package_prefix", "com.roblox")
                            if pkg[0] == f"{prefix}.client":
                                pkg_display = 'Global 🌍'
                            elif pkg[0] == f"{prefix}.client.vnggames":
                                pkg_display = 'VNG 🇻🇳'
                            else:
                                pkg_display = pkg[0]
                            print(f"  - {pkg_display}")
    ...
    while True:
        ...
        if setup_type == "1":
            try:
                ...
                # NEW: Package selection functionality
                print("\n🎯 Package Selection Mode")
                package_list = PackageSelector.display_package_list(globals()["accounts"], server_links)
                selected_packages = PackageSelector.select_packages(package_list)
                
                if not selected_packages:
                    print("\033[1;31m[ Shouko.dev ] - No packages selected. Returning to menu.\033[0m")
                    input("\033[1;32mPress Enter to return...\033[0m")
                    continue

                # Filter server_links to only include selected packages
                filtered_server_links = []
                for package_name, user_id, server_link in selected_packages:
                    filtered_server_links.append((package_name, server_link))
                ...
                RobloxManager.kill_roblox_processes()
                time.sleep(5)
                Runner.launch_package_sequentially(filtered_server_links)
                globals()["is_runner_ez"] = True

                for task in [
                    (Runner.monitor_presence, (filtered_server_links, stop_main_event)),
                    (Runner.force_rejoin, (filtered_server_links, force_rejoin_interval, stop_main_event)),
